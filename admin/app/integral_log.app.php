...
//            INTEGRAL_GOODS => Lang::get('integral_goods'), #兑换礼品扣除白积分类型
    ...
    function set() {
        ...
        if (!IS_POST) {
            ...
            $data['integral_reg'] = empty($_POST['integral_reg']) ? 0 : intval($_POST['integral_reg']);        #注册获得白积分
            $data['integral_login'] = empty($_POST['integral_login']) ? 0 : intval($_POST['integral_login']);  #登录获得白积分
            $data['integral_recom'] = empty($_POST['integral_recom']) ? 0 : intval($_POST['integral_recom']);  #推荐获得白积分
            $data['integral_buy'] = empty($_POST['integral_buy']) ? 0 : round($_POST['integral_buy'], 2);   #购买获得白积分
            $data['integral_seller'] = empty($_POST['integral_seller']) ? 0 : round($_POST['integral_seller'], 2);   #用户购买抵扣白积分 比例
