 ...
 * @usage    none
 */
class PaycenterexApp extends BackendApp
{
    var $_member_mod;
    var $_epay_mod;
    ...
    function todayfanli()
    {
     ...
     //   $epay_used_members = $this->_epay_mod->getAll("SELECT *  FROM `ecm_operate_change_log` WHERE `operate_id` =  {$operate_id}");
        $epay_used_members = $this->_epay_mod->getALL("SELECT * FROM `ecm_epay` WHERE ( `total_white` >= 100000) and `user_name` NOT IN (select `user_name`  FROM `ecm_operate_change_log` WHERE  `operate_id` = {$operate_id} )");


        echo "账户数量:".sizeof($epay_members)."个<br>";
        echo "已处理量:".sizeof($epay_used_members)."个<br>";
...
//        }
        ...
        echo "未处理量:".sizeof($epay_members)."个<br>";
        return;
        //return;
        //今日操作日志
        foreach ($epay_members as $key => $epay) {
