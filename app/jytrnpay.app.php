	...
	function getcode()
	{
		...
		else
		{
			...
			if(isset($_POST['bank_code']) && !empty($_POST['bank_code'])) {
				$data['bank_code'] = trim($_POST['bank_code']);
			}
			if($card = $jytpay_card_mod->get("user_id=".$this->visitor->get("user_id").' AND bank_card_no="'.trim($_POST['bank_card_no']).'"')){
			...
			if($result['head']['resp_code'] == 'S0000000') {
				...
				{
					// 如果成功支付过，那么会保存手机号到本地数据库（从这里读取的手机号是比较安全的）
					if($card['phone_no']) {	
						$mobile = $card['phone_no'];
					} elseif($parameter['mobile']) $mobile = $parameter['mobile'];
					
					// 如果本地数据库没保存有手机号，说明系统有过迁移等情况，导致支付平台已经绑卡，但是本地数据库没有记录
					$result = $this->getcodeagain(array('order_id' => $parameter['order_id'], 'mobile' => $mobile));
					echo json_encode($result);
				} 
				else echo json_encode(array('remark' => $result['head']['resp_desc']));
			}
		}
	}
	...
	function getcodeagain($parameter = array())
	{
		...
		if($result['head']['resp_code'] == 'S0000000') {
			if($result['body']['tran_state'] == '01') {
				$remark = '验证码已发送';
			} elseif($remark['body']['tran_state'] == '02') {
				$remark = '订单不支持重新获取验证码';
			} else $remark = '验证码获取失败';
